...
namespace sdds {
	...
	Name::Name(const Name& toCopyFrom) {
		if (toCopyFrom.m_middleName != nullptr && toCopyFrom.m_lastName != nullptr) {
			set(toCopyFrom.m_firstName, toCopyFrom.m_middleName, toCopyFrom.m_lastName);
		}
	...
	Name& Name::set(const char* first, const char* middle, const char* last) {
		setFirst(first);
		setMiddle(middle);
		setLast(last);
		return *this;
	...
	std::ostream& Name::display(std::ostream& ostr)const {
		...
		else if (m_middleName == nullptr && m_lastName != nullptr) {
